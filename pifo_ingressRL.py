...
"""
This version of the PIFO accepts:
  - rank to enqueue packet into scheduling PIFO
  - wall clock time to enqueue into shaping PIFO
To dequeue:
  - wait until wall clock time at head of shaping
    PIFO then dequeue from both PIFOs
"""

class PIFO_ingressRL(HW_sim_object):
    def __init__(self, env, period, r_in_pipe, r_out_pipe, w_in_pipe, w_out_pipe, rank_w_in_pipe, rank_w_out_pipe, rank_r_in_pipe, rank_r_out_pipe, buf_size=None, num_queues=1):
        ...
        """
        super(PIFO_ingressRL, self).__init__(env, period)
    ...
    def write_pifo_sm(self):
        ...
        while not self.sim_done:
            # wait to receive incoming data
            (rank, send_time, q_id, pkt) = yield self.rank_r_out_pipe.get()
            heappush(self.sched_vals, (rank, q_id, pkt))
            heappush(self.shape_vals, send_time)
    ...
    def read_sm(self):
        ...
        """
        while not self.sim_done:
            if len(self.shape_vals) > 0:
                # wait until time reaches the value at the head of the shaping PIFO
                while self.env.now < nsmallest(1, self.shape_vals)[0]:
                    yield self.wait_clock()

                # remove from shaping and scheduling PIFOs
                heappop(self.shape_vals)
                assert(len(self.sched_vals) > 0)
                (rank, q_id, pkt) = heappop(self.sched_vals)
